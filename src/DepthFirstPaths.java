...
public class DepthFirstPaths {
    ...
    public void nonrecursiveDFS(Graph G) {
        ...
        for (int v = 0; v < G.V(); v++)
            adj[v] = G.adj(v).iterator();   //array[v] von iteratoren über alle adjazenten knoten von knoten v aus
        ...
        distTo[s] = 0;
        stack.push(s);
        //bearbeite bis alle erreichbaren knoten bearbeitet wurden
        while (!stack.isEmpty()) {
            //get zu bearbeitender knoten
            int v = stack.peek();
            //wenn knoten adjazente knoten hat, guck an, sonst beende die bearbeitung dieses knotens
            if (adj[v].hasNext()) {
                int w = adj[v].next();
                //wenn der nachbar noch nicht besucht wurde, bearbeite ihn als nächstes
                if (!marked[w]) {
